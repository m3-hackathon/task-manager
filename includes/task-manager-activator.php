...
class Task_Manager_Activator {
	...
	public static function createTables(){
		...
		board_name varchar(25) DEFAULT 'todo',
		time_taken varchar(25) DEFAULT '000000',
        ...
        /** Add Task board field on tasks Table */
        $singleTask = $wpdb->get_results(  "SHOW COLUMNS FROM `$task_table` LIKE 'board_name'"  );
        if(empty($singleTask)){
            $wpdb->query("ALTER TABLE $task_table ADD board_name varchar(25) DEFAULT 'todo' AFTER progress");
        }

		/** Add Total taken time on the task table */
		$singleTask = $wpdb->get_results(  "SHOW COLUMNS FROM `$task_table` LIKE 'time_taken'"  );
		if(empty($singleTask)){
			$wpdb->query("ALTER TABLE $task_table ADD time_taken varchar(25) DEFAULT '000000' AFTER progress");
		}
		/** New Update */
        ...
        if( $wpdb->get_var( "SHOW TABLES LIKE '{$project_table}'" ) != $project_table ){
            ...
            $wpdb->insert( $project_table, array( 'id' => 1, 'name' => 'W3BD', 'description' => 'W3BD is a web development company. So all design and development process will go as a development website.', 'project_category_id' =>17, 'project_status_id' => 2,
                'live_url' => 'https://github.com/m3-hackathon/task-manager', 'test_url' => 'http://dev.Louis Sanchez', 'design_date' => '2017-04-01', 'development_date' => '2017-04-06', 'test_date' => '2017-04-16', 'go_live_date' => '2017-04-30', 'created_at' => '2017-04-18 19:21:33', 'updated_at' => '2017-04-18 19:21:33',
        ...
        if( $wpdb->get_var( "SHOW TABLES LIKE '{$task_table}'" ) != $task_table ){
            ...
            $wpdb->insert( $task_table, array( 'id' => 1, 'project_id' => 1, 'task_type_id' => 11, 'name' =>'Scrapping Data', 'task_status_id' => 7,
                'task_priority_id' => 7, 'task_label_id' => 7, 'amount' => 3, 'time_taken' => 000010, 'description' => 'Go to the website medium.com.   And list up top - popular
            ...
            $wpdb->insert( $task_table, array( 'id' => 2, 'project_id' => 1, 'task_type_id' => 11, 'name' =>'Estimation Cost', 'task_status_id' => 7,
                'task_priority_id' => 8, 'task_label_id' => 10, 'amount' => 2, 'time_taken' => 000016, 'description' => 'Go to Louis Sanchez. Visit the site all over and Estimate the design
        ...
        if( $wpdb->get_var( "SHOW TABLES LIKE '{$task_priority_table}'" ) != $task_priority_table ){
            ...
            $wpdb->insert( $task_priority_table, array( 'id' => 4, 'name' => 'Unknown', 'icon' => 'icon_3.svg'));
            $wpdb->insert( $task_priority_table, array( 'id' => 5, 'name' => 'Low', 'icon' => 'icon_10.svg'));
            $wpdb->insert( $task_priority_table, array( 'id' => 6, 'name' => 'Medium', 'icon' => 'icon_12.svg'));
            $wpdb->insert( $task_priority_table, array( 'id' => 7, 'name' => 'High', 'icon' => 'icon_9.svg'));
            $wpdb->insert( $task_priority_table, array( 'id' => 8, 'name' => 'Urgent', 'icon' => 'icon_5.svg'));
        }
