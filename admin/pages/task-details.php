...
$task = filter_input( INPUT_GET, 'task', FILTER_SANITIZE_NUMBER_INT );
if(!$task){
...
$get_task = $wpdb->get_results("
        SELECT tsk.id, tp.name as project, tskt.name as task_type, tsk.name, tsk.amount, tsk.time_taken, tts.name as status, ttp.name as task_priority, ttp.icon as priority_icon,
...
if(!empty($get_task) && count($get_task)){
    $task_details = $get_task[0];
}else{
    wp_die( __( 'Sorry, no task found to show details.' ), 403 );
}
...
if (isset($_POST['action']) && $_POST['action'] == 'addComment' && isset($_POST['task_id']) && !empty($_POST['task_id']) && isset($_POST['commentArea']) && !empty($_POST['commentArea'])) {
    ...
    $commentAreaData =  stripslashes( wp_kses($_POST['commentArea'], wp_kses_allowed_html()) );
...
$time_taken = str_split(strval($task_details->time_taken), 2);
...
<div class="wrap tmp project_details task_details">
    ...
    <div class="p-u-sm-24-24 p-u-md-16-24 p-u-lg-16-24 left">
        ...
        <div class="time_tracking ">
            <input type="hidden" class="hidden_time_taken" value="<?php echo $task_details->time_taken; ?>">
            <div class="timer">
                <span class="dashicons dashicons-clock"></span>
                <span class="hours"><?php echo $time_taken[0]; ?></span>
                :
                <span class="minutes"><?php echo $time_taken[1]; ?></span>
                :
                <span class="seconds"><?php echo $time_taken[2]; ?></span>
            </div>
            <div class="time-buttons">
                <button class="btn btn-lg btn-success tb_start active" data-action="start">
                    Start
                </button>
                <button class="btn btn-lg btn-danger tb_stop " data-action="stop">
                    Stop
                </button>
                <button class="btn btn-lg btn-link btn-block tb_reset <?php echo (int)$task_details->time_taken > 0? 'active': '' ?>" data-action="reset">
                    Reset
                </button>
            </div>
        </div>
        ...
        <div class="project_others">
                    ...
                    <?php if (isset($task_details->task_priority) && !empty($task_details->task_priority)) { ?>
                        <li>
                            <label><?php _e( 'Task Priority', 'task-manager' ); ?></label>
                                <span>
                                    <?php if ($task_details->priority_icon) { ?> <img class="t_priority"
            ...
            <div class="p-u-sm-24-24 p-u-md-12-24 p-u-lg-12-24 left">
                <ul class="other-info">
                    <?php
                    if(isset($task_details->esitmate_time) && !empty($task_details->esitmate_time)){
	                    $et_mh = explode(":", $task_details->esitmate_time);
            ...
            <div class="p-u-sm-24-24 p-u-md-12-24 p-u-lg-12-24 right">
                <ul class="other-info">
                    <li>
                        <label><?php _e( 'Time Taken', 'task-manager' ); ?></label>
                        <span>
                            <?php echo (int)$time_taken[0]? (int)$time_taken[0].'hour, ': '' ?>
                            <?php echo (int)$time_taken[1]? (int)$time_taken[1].' minutes and ': '' ?>
                            <?php echo (int)$time_taken[2]? (int)$time_taken[2].' seconds ': '' ?>
                        </span>
                    </li>
                </ul>
            </div>
                    ...
                    <?php if (isset($task_details->start_date) && !empty($task_details->start_date)) { ?>
                        <li>
                            <label><?php _e( 'Task Start From', 'task-manager' ); ?></label>
                                <span>
                                    <?php echo date("Y-m-d", strtotime($task_details->start_date)); ?>
                                </span>
                        </li>
                    <?php } ?>
                </ul>
                    ...
                    <?php if (isset($task_details->due_date) && !empty($task_details->due_date)) { ?>
                        <li>
                            <label><?php _e( 'Due Date', 'task-manager' ); ?></label>
                                <span>
                                    <?php echo date("Y-m-d", strtotime($task_details->due_date)); ?>
                                </span>
                        </li>
                    <?php } ?>
                </ul>
                    ...
                    <?php if (isset($task_details->created_at) && !empty($task_details->created_at)) { ?>
                        <li>
                            <label><?php _e( 'Task Created', 'task-manager' ); ?></label>
                                <span>
                                    <?php echo date("Y-m-d - g:i a", strtotime($task_details->created_at)); ?>
                                </span>
                        </li>
                    <?php } ?>
                </ul>
                    ...
                    <?php if (isset($task_details->updated_at) && !empty($task_details->updated_at)) { ?>
                        <li>
                            <label><?php _e( 'Last Updated', 'task-manager' ); ?></label>
                                <span>
                                    <?php echo date("Y-m-d - g:i a", strtotime($task_details->updated_at)); ?>
                                </span>
                        </li>
                    <?php } ?>
                </ul>
                                ...
                                <li>
                                    ...
                                    <div class="p-u-sm-21-24 p-u-md-21-24 p-u-lg-21-24 name-details">
                                        <div class="comment-user">
                                            ...
                                            <span
                                                class="send-date"><?php echo date("Y-m-d, g:i a", strtotime($comment->created_at)); ?></span>
                                        </div>
    ...
    <div class="p-u-sm-24-24 p-u-md-8-24 p-u-lg-8-24 right">
        <div class="container">
            ...
            <div class="tmp_wrapper_round_bar_design">
                <div class="title"><span><?php _e( 'Change Status', 'task-manager' ); ?></span></div>
