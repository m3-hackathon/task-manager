  ...
  this.jKanban = function () {
    ...
    this.addBoards = function (boards, isInit) {
      ...
      for (var boardkey in boards) {
        ...
        boardNode.classList.add('kanban-board', board.id)
        //set style
        if (self.options.responsivePercentage) {
    ...
    function __buildItemCard(item) {
      var amount = 'amount' in item ? item.amount : '';
      var date = 'due_date' in item ? item.due_date : '';
      var project_name = 'project_name' in item ? item.project_name : '';
      var title = 'title' in item ? item.title : '';
      var priority_icon = 'priority_icon' in item ? item.priority_icon : '';
      var progress = 'progress' in item ? item.progress : '';
      var result =  '<div class=\'t_top\'>'
                    + '<div class=\'t_priority\'><img src=\'' + priority_icon + '\' alt=\'Priority\' /></div>'
                    + '<div class=\'t_date\'>$' + amount + '</div>'
                  + '</div>'
                  + '<div class=\'t_title\'>' + title + '</div>'
                  + '<div class=\'t_bottom\'>'
                    + '<div class=\'t_project\'>' + project_name + '</div>'
                    + '<div class=\'t_date\'>' + date + '</div>'
                  + '</div>'

      if (self.options.itemHandleOptions.enabled) {
        result =  '<div class=\'item_handle drag_handler\'></div>' +
            '<div class=\'t_top\'>' +
              '<div class=\'t_t_left\'>' +
                '<div class=\'t_priority\'><img src=\'' + priority_icon + '\' alt=\'Priority\' /></div>' +
                '<div class=\'t_date\'>$' + amount + '</div>' +
              '</div>' +
              '<div class=\'t_t_right\'>' +
                '<div role=\'progress-bar-parcent-cover\' aria-valuenow=\''+ progress +'\' aria-valuemin="0" aria-valuemax="100" style=\'--value:'+ progress + '\'></div>'+
              '</div>' +
            '</div>' +
            '<div class=\'t_title\'>' + title + '</div>' +
            '<div class=\'t_bottom\'>' +
