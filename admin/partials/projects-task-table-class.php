...
class Task_Manager_Table extends WP_List_Table {
    ...
    public function getAccessId(){
        ...
        SELECT tug.task_access_id as task_access
        ...
        WHERE tu.user_id = $owner_id
         ");
            if( !empty($getAccessId) ){
                $task_access = $getAccessId[0]->task_access;
            }else{
                $task_access = 0;
            }
        }
...
//    }
    ...
    function column_action($item){
        $task_access = $this->getAccessId();

        if ( in_array($task_access, array(12,11, 13, 16)) ) {
    ...
    function column_project_name($item){
        return  sprintf('<a href="?page=project-details&project=%s">%s</a>',$item['project_id'],$item['project_name']);
    }
    ...
    function get_columns(){
        $columns = array(
            ...
            'due_date'    => __( 'Dui Date', 'task-manager' ),
    ...
    function prepare_items() {
         ...
         * Project Query Data
         */
        if(isset($_REQUEST['project']) && !empty($_REQUEST['project'])) { $project = $_REQUEST['project'] ;}else{ $project =  null;}
//
        $tasks = $wpdb->get_results("
            SELECT tsk.id, tsk.project_id, tsk.name, tts.name as task_status, ttp.name as task_priority, 
