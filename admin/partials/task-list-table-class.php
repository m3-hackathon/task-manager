...
//            'created_at'    => __( 'Created At', 'task-manager' ),
    ...
    function prepare_items($search = '') {
	    ...
	    $query = "SELECT tsk.id, tsk.project_id, tp.name as project_name, tsk.name, tsk.amount, tsk.board_name, tts.name as task_status,ttp.name as task_priority, 
        ...
        }elseif(in_array($task_access, [12,13])){
	        if(!empty($search)){
		        $query .= " AND (tsk.name LIKE '%{$search}%')";
	        }
        }else{
