...
class Task_Manager_Admin {
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( 'tmp-admin-board-style', plugin_dir_url( __FILE__ ) . 'css/jkanban.css', array(), $this->version, 'all' );
...
//		wp_enqueue_style( 'min-grid-css', plugin_dir_url( __FILE__ ) . 'css/grid-min.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( $this->Task_Manager, 'ajax_object', array( 'ajax_url' => admin_url( 'admin-ajax.php' ), 'user_id' => get_current_user_id() ) );
...
//			add_submenu_page('projects', __( 'Boards', 'task-manager' ), __( 'Boards', 'task-manager' ), 'read', 'boards', 'task_board_file_function', 2);
		}
...
//			add_submenu_page('tasks', __( 'Boards', 'task-manager' ), __( 'Boards', 'task-manager' ), 'read', 'boards', 'task_board_file_function', 2);
		}

		// Editor Access
		if ( in_array($tp_access['task'], array(12,13,16))) {
			add_submenu_page('tasks', __( 'Add New', 'task-manager' ), __( 'Add New', 'task-manager' ), 'read', 'task-new', 'new_task_file_function', 1);
		}

		// Only Admin Access
		if ( in_array($tp_access['task'], array(13,16)) ) {
			add_submenu_page('tasks', __( 'Task Types', 'task-manager' ), __( 'Task Types', 'task-manager' ), 'read', 'task-types', 'task_type_file_function', 3);
			add_submenu_page('tasks', __( 'Task Status', 'task-manager' ), __( 'Task Status', 'task-manager' ), 'read', 'task-status', 'task_status_file_function', 4);
			add_submenu_page('tasks', __( 'Task Priorities', 'task-manager' ), __( 'Task Priorities', 'task-manager' ), 'read', 'task-priorities', 'task_priority_file_function', 5);
			add_submenu_page('tasks', __( 'Task Labels/Tags', 'task-manager' ), __( 'Task Labels/Tags', 'task-manager' ), 'read', 'task-labels', 'task_label_file_function', 6);
		}
		...
		if ( in_array($tp_access['task'], array(11,12,13,14,16)) ) {
			...
			add_menu_page( __( 'Boards', 'task-manager' ), __( 'Boards', 'task-manager' ), 'read', 'boards', 'task_board_file_function', 'dashicons-desktop', 21);
		}
...
//		    add_options_page(__( 'Task Manager', 'Task_Manager' ), __( 'Task Manager', 'Task_Manager' ), 'read','task-manager', 'task_manager_setting_func');
//        }
		...
		function task_board_file_function(){
			include plugin_dir_path( __FILE__ ) . 'pages/task-boards.php';
		}
	...
	public function tmp_dashboard_widgets(){
		...
		function tmp_glance_items( $items = array() ) {
			...
			$items[] = sprintf( '<a href="admin.php?page=boards" title="Boards">%s Boards</a>', 4);
			return $items;
		}
	}
	...
	public function tmp_modify_admin_menu_badge(){
		...
		if( $get_project_count_badge == 'yes' ){
			$menu['20.56483'][0] = $project_label;
		}

		if( $get_task_count_badge == 'yes' ){
			$menu['20.24315'][0] = $task_label;
		}
		...
		if(isset($_POST['hidden_csv'])){
			...
			if(!empty($_FILES['upload']['name']) && $extension == 'csv'){
				...
				if($file_handle) {
					...
					while ( ( $line = $csv_reader->get_row() ) !== NULL ) {
						...
						if(empty($task)){
							...
							if(empty($taskStatusQuery)){
								$wpdb->insert($wpdb->prefix . "tmp_task_priorities", array('name' => $taskdata['task_priority'], 'icon' => 'icon_3.svg'));
                            ...
                            $taskDetails =  stripslashes( wp_kses($_POST['description'], wp_kses_allowed_html()) );
				...
				add_action( 'admin_notices', function () use ($error){ ?>
                    <div class="updated notice">
                        <p><h3 style='color: red;'><?php _e( "Invalid Extension", 'task-manager' ); ?></h3></p>
                    </div>
				<?php });
			}
        }
    ...
    public function task_time_taken(){
	    global $wpdb;
	    ...
	    if($task_id && $time_taken){
		    $wpdb->update($task_table, ['time_taken' => $time_taken], ['id' => $task_id]);
	    }
    ...
    public function get_user_by_project_id(){
        ...
        $project_users = $wpdb->get_results("SELECT tpu.user_id FROM {$wpdb->prefix}tmp_project_users as tpu WHERE tpu.project_id = {$project_id}", ARRAY_A);
        $pu_arr = [];
        $tmp_users = null;
        ...
        if(!empty($tmp_users)){
            echo json_encode(['success' => true, 'data' => $tmp_users]);
        }else{
	...
	public function submit_board(){
        global $wpdb;

        if($_POST['task_data'] && count($_POST['task_data'])){
            $board_name = $_POST['task_data']['board_name'];
            $task_id = $_POST['task_data']['task_id'];
            $task_table = $wpdb->prefix . 'tmp_tasks';
            if($board_name && $task_id){
                $wpdb->update($task_table, ['board_name' => $board_name], ['id' => $task_id]);
                echo 'done!';
            }else{
                echo 'error!';
            }
...
//        $phpmailer->Username = 'task.manager.m3.hackathon@gmail.com'; // if required
//        $phpmailer->Password = 'www.louie-sanchez.com'; // if required
...
//		);
		...
		add_settings_field(
			...
			__('Enable Multiple User Assign Option to group', 'task-manager'),
		...
		add_settings_field(
			'tmp_only_selected_users_can_assign_for_a_task',
			__('Only selected users for the project can be assigned for a task', 'task-manager'),
			array($this, 'tmp_only_selected_users_can_assign_for_a_task_function'),
			'task-manager-options',
			'Task_Manager_settings_section'
		);
...
//        );
		...
		add_settings_field(
			'tmp_boards',
			__('Boards', 'task-manager'),
			array($this, 'tmp_boards_callback_function'),
			'task-manager-options',
			'Task_Manager_settings_section'
		);
		...
		register_setting( 'task-manager-options', 'tmp_only_selected_users_can_assign_for_a_task' );
		register_setting( 'task-manager-options', 'tmp_default_project_status' );
		register_setting( 'task-manager-options', 'tmp_custom_css' );
		register_setting( 'task-manager-options', 'tmp_currency_symbol' );

		// Task Board
		register_setting( 'task-manager-options', 'tmp_boards' );
	...
	function tmp_only_selected_users_can_assign_for_a_task_function() {
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/partials/settings/tmp_only_selected_users_can_assign_for_a_task.php';
	}
	...
	function tmp_boards_callback_function() {
		require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/partials/settings/tmp_boards.php';
	}
	...
	public function add_admin_menu() {
		add_options_page( 'Task Manager', __('Task Manager', 'task-manager'), 'manage_options', 'task-manager-options', array($this, 'create_admin_interface'));
	}
